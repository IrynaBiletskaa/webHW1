...
class AddressBook(UserList):
    ...
    def __str__(self):
        ...
        return '\n'.join(result)
# ось ця функція була у початковому варіанті
    # def __next__(self):
    #     phones = []
    #     self.counter += 1
...
#   ось ця функція була у початковому варіанті
    ...
    #             result.append(account)
    #     if not result:
    #         self.ui.print_info('There is no such contact in address book!')
    #         return []
    #     return result
    def search(self, pattern, category):
        result = []
        category_new = category.strip().lower().replace(' ', '')
        pattern_new = pattern.strip()

        for account in self.data:
            if category_new == 'phones':
                for phone in account['phones']:
                    if phone.lower().startswith(pattern_new.lower()):
                        result.append(account)
            elif category_new == 'birthday':
                if account['birthday'] and account['birthday'].strftime("%d/%m/%Y") == pattern_new:
                    result.append(account)
            elif account[category_new].lower().replace(' ', '') == pattern_new.lower():
                result.append(account)
    ...
    def edit(self, contact_name, parameter, new_value):
        ...
        try:
            ...
            self.ui.print_info(
                'Incorrect parameter! Please provide correct parameter')
        except NameError:
            self.ui.print_info('There is no such contact in address book!')
        else:
            ...
            return True
        return False
# ця функція була в початковому варіанті
    ...
    def remove(self, pattern):
        flag = False
        for account in self.data:
            if account['name'] == pattern or pattern in account['phones']:
                self.data.remove(account)
                self.log(f"Contact {account['name']} has been removed!")
                flag = True
                break
